...
## 2026-07-15 — A hard-killed Codex run stays `status: running` and is injected by the hook indefinitely
**Why:** Phase 6's incremental `run.json` writes `running` at start and a terminal status only on a clean exit; `render_hook_block` ages out (12h cutoff) only *terminal* runs. So a run whose process is SIGKILLed mid-task leaves `run.json` stuck at `running`, and the UserPromptSubmit hook keeps injecting it as live forever — indistinguishable from an actually-live run. Spec-compliant (the spec ages out only terminal runs), and benign in practice (the next invocation over that run-dir rewrites the status; `--status` shows RUNNING honestly), but it slightly undercuts the design's "distinguish an in-progress run from a dead one" goal. Surfaced by the Phase 6 final integration review as an observation, not a finding.
**From:** Phase 6 session-awareness final review (2026-07-15)
**Follow-up:** revisit-if-killed-runs-cause-confusion; candidate fix is a liveness signal (pid + `os.kill(pid, 0)` check, or a heartbeat mtime with a cutoff for `running` runs too) so the hook/`--status` can mark a stale `running` run as likely-dead.

...
- **`--notify CMD`** — the runner fires a one-line summary on each of four events: `task-passed` (a task passes, one per task), `escalated` (a task hits the rework cap / final-review findings — a halt), `contract-error`, and `completed` (whole run done). `fire_notify(event, summary, cmd)`, invoked via `subprocess.Popen` fire-and-forget: never blocks the exit path, never changes the exit code; a broken CMD is a stderr warning, not an error. Call sites: one `task-passed` in the per-task pass branch, `escalated` at the escalation exit and on final-review findings, `contract-error` in `main()`, `completed` at the successful end. A user CMD receives `event` and `summary` as appended argv (so it can route progress vs. halts however it likes). No CMD given: macOS default is a modal `osascript 'display alert'` for every event; non-darwin default writes the summary to stderr and fires nothing. `FORGE_NOTIFY_DISABLE=1` silences the default modal (set suite-wide in tests). On a multi-task run the final task fires both its `task-passed` and the run's `completed`.
